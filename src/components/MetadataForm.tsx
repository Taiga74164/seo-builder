...
export const MetadataForm: React.FC<IMetadataFormProps> = ({ checkedItems, formData, onCheck, onInputChange }) => {
	...
	const categoryTabs = Object.entries(METADATA_CATEGORIES).map(([key, config]) => ({
		key,
		label: config.title,
		icon: config.icon,
	}));

	const currentCategory = METADATA_CATEGORIES[activeCategory];
		...
		<div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
			...
			<div className="mb-8">
				<div className="flex items-center mb-4">
					<currentCategory.icon className="w-5 h-5 mr-2 text-blue-500" />
					<div>
						<h3 className="text-lg font-medium text-gray-800 dark:text-gray-200">{currentCategory.title}</h3>
						<p className="text-sm text-gray-600 dark:text-gray-400">{currentCategory.description}</p>
					</div>
				</div>

				<div className="space-y-4">
					{currentCategory.items.map((item) => {
						const isChecked = checkedItems[`${activeCategory}-${item.key}`] || false;
						...
						return (
							<div key={item.key} className="space-y-2">
								<div className="flex items-start space-x-3">
									<Checkbox
										id={`${activeCategory}-${item.key}`}
										checked={isChecked}
										onCheckedChange={(checked) =>
											onCheck(activeCategory, item.key, checked as boolean)
										}
										className="mt-0.5"
									/>
									<div className="flex-1">
										<div className="flex items-center space-x-2">
											<label
												htmlFor={`${activeCategory}-${item.key}`}
